...
Act as the orchestrator using `model: "gpt-5.6-sol"` with `reasoning_effort: "medium"`. Launch variable, domain-specific workers using `model: "gpt-5.6-terra"` with `reasoning_effort: "high"`; set `fork_turns: "none"` and give each only the contract, relevant source/test paths, test command, and owned output paths.
