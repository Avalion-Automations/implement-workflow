---
name: red-team
description: Run an adversarial, read-only software review through domain-specialist subagents and a final finding validator. Use only when the user explicitly invokes `$implement:red-team` or requests adversarial review, defect hunting, or independent validation of an implementation or handoff.
---
...
When invoked by `$implement:build`, read [references/orchestration-contract.md](references/orchestration-contract.md) and use its fresh-context, manifests, receipt, model fallback, budget, and telemetry rules.
...
[$implement:bug-finding-review](../bug-finding-review/SKILL.md), selecting
...
Return one `red-1.json`. Build does not ask Red to re-review a Fixer candidate; Fixer’s scoped Judge is the post-repair gate. Standalone `$implement:red-team` remains the deep, open-ended adversarial workflow.
