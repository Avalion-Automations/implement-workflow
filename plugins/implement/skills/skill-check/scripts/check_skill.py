...
class Checker:
    ...
    def _check_local_references(self) -> None:
        ...
        for candidate in sorted(set(candidates)):
            ...
            try:
                target_path.relative_to(self.skill_path.resolve())
            except ValueError:
    ...
    def _check_openai_metadata(self) -> None:
        metadata_path = self.skill_path / "agents" / "openai.yaml"
        if not metadata_path.exists():
            self.add("warning", "ui-metadata", "agents/openai.yaml is absent; create it for skill discovery UI.")
            return
        try:
            metadata = yaml.safe_load(metadata_path.read_text(encoding="utf-8")) or {}
        except yaml.YAMLError as exc:
            self.add("error", "ui-metadata-yaml", f"Invalid agents/openai.yaml: {exc}")
            return
        interface = metadata.get("interface") if isinstance(metadata, dict) else None
        if not isinstance(interface, dict):
            self.add("error", "ui-metadata-shape", "agents/openai.yaml must contain an interface mapping.")
            return
