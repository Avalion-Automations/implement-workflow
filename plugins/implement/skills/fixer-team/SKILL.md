...
Write every eligible ID to `fixer-1.json` with `result: fixed|not-reproducible|needs-context|blocked`. Any result other than `fixed` blocks Build readiness but preserves the best candidate for explicit standalone deep work. Standalone `$implement:fixer-team` retains the iterative per-finding loop below.
